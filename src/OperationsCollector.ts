...
export class BaseOperationsCollector implements OpenAPIVisitor {
  ...
  _visitOperation(
    ...
    const resources = operation.tags!!.map((tag: string) =>
      this.resourceParser.value({ name: tag })
    );
  ...
  parseFields(operation: OpenAPIV3.OperationObject, context: OperationContext) {
    ...
    if (operation.parameters) {
      ...
      // Get all parameters
      const allParameters = this.n8nNodeProperties.fromParameters(
    ...
    // Handle request body
    try {
      const bodyFields = this.n8nNodeProperties.fromRequestBody(
      ...
      const notice: INodeProperties = {
        displayName: `${context.method.toUpperCase()} ${
          context.pattern
        }<br/><br/>${msg}`,
        name: "operation",
        type: "notice",
